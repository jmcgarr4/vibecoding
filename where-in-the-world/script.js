...
// ── Load images ───────────────────────────────────────────────────────────────
// Primary: fetch landmark-images.json (self-hosted, generated by GitHub Actions).
// Fallback: Wikipedia REST API (works on desktop; may fail on iOS Safari).
async function loadImages() {
  // Try self-hosted manifest first (same-origin, always works on iOS Safari)
  try {
    const res = await fetch('landmark-images.json');
    if (res.ok) {
      const map = await res.json();
      LANDMARKS.forEach(lm => { lm.image = map[lm.wiki] ?? ''; });
      return;
    }
  } catch (_) { /* manifest not generated yet — fall through */ }
